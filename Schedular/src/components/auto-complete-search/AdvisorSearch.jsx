...
export default function AdvisorSearch({ setFilters }) {
  const loadOptions = async (inputValue) => {
    ...
    try {
      const response = await axios.get('http://localhost:8000/api/v1/advisors/searchAdvisor', {
        params: { search: inputValue }
      });

      const advisors = response.data.data.advisors;
      return advisors.map(advisor => ({
        label: `${advisor.title} ${advisor.fullname}`,
        value: advisor._id,
      }));
    } catch (error) {
  ...
  const handleChange = (selectedOption) => {
    if (selectedOption) {
      ...
      setFilters(prev => ({ ...prev, advisor: '' }));  // Clear selection
    }
  };
